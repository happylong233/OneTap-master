...
interface ContactDao {
    ...
    @Query(
        "SELECT * FROM contacts WHERE isVisibleOnHome = 1 ORDER BY `order` ASC, createdAt ASC"
    )
    fun getHomeContacts(): Flow<List<Contact>>

    @Query("SELECT * FROM contacts WHERE isEmergency = 1 ORDER BY `order` ASC")
